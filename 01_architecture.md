...
ЭТАП 1: Сбор инструментов (REST API)
  ...
  Для каждой пары извлекаем:
    ...
    - base_asset: "BTC"
    - quote_asset: "USDT"
    - status: торгуется ли (TRADING / ONLINE)
    - min_qty, tick_size (для будущего Order Manager)

  Нормализация:
    exchange_symbol → normalized_name: "BTC-USDT"
    Правило: "{BASE}-{QUOTE}" в upper case


ЭТАП 2: Формирование глобального списка
  ┌──────────────────────────────────────────────────────────────┐
  │  Объединить все пары со всех 8 источников                    │
  │  Дедупликация по normalized_name                             │
  │  Присвоить symbol_id: 0, 1, 2, ...                          │
...
ЭТАП 4: WS-валидация
  ┌──────────────────────────────────────────────────────────────┐
  │  Для каждого из 8 источников:                                │
  │    1. Подключиться к WS                                      │
  │    2. Подписаться на ВСЕ пары батчами                        │
  │    3. Ждать 30 секунд                                        │
  │    4. Пометить пары, от которых НЕ пришло ни одного update   │
  │    5. Удалить невалидные пары из списков и направлений       │
  │                                                              │
  │  Типичные причины отсутствия ответа:                         │
  │    - Пара приостановлена (maintenance)                       │
  │    - Пара с нулевой ликвидностью                             │
  │    - Неправильный формат подписки                             │
  │    - Пара существует в REST но не в WS                        │
  └──────────────────────────────────────────────────────────────┘


ЭТАП 5: Генерация конфигов
  ┌──────────────────────────────────────────────────────────────┐
  │  Записать:                                                    │
