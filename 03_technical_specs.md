...
## 1.1.5 Discovery Error Handling

### Graceful Degradation Strategy

```rust
enum DiscoveryError {
    // Fatal errors - немедленный exit
    ConfigNotFound { path: PathBuf },
    InvalidConfig { reason: String },
    WriteError { path: PathBuf, source: std::io::Error },
    
    // Recoverable errors - продолжаем с partial results
    RestFailed { exchange: Exchange, market: Market, source: reqwest::Error },
    ValidationFailed { source: SourceId, reason: String },
    
    // Threshold errors - exit если слишком много failures
...
### Минимальные пороги работоспособности

- **REST fetch:** минимум 6/8 источников успешны
- **WS validation:** минимум 6/8 источников валидны
- **Per-direction:** минимум 1 пара (если 0 → direction disabled, не fatal)
...
**WS validation:**
- Per-batch timeout: 90 секунд
- Collect duration: 30 секунд
- Idle timeout: 10 секунд
- No retries (если batch failed → помечаем invalid)

### Logging

```rust
// Fatal errors
error!("Config not found: {:?}", path);  → exit(1)

// Recoverable errors
warn!("REST fetch failed for {:?}/{:?}, retry {}/{}", exchange, market, attempt, max);

// Success with degradation
info!("Discovery completed: {}/8 sources successful", successful);
if successful < 8 {
    warn!("Running with degraded universe: {} sources unavailable", 8 - successful);
}
```
